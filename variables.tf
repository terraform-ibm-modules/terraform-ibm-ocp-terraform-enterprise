...
variable "existing_cis_instance_name" {
  type        = string
  description = "Existing IBM Cloud Internet Service instance name providing the support for the base domain of custom hostname to use for Terraform Enterprise instance. It is required to configure a custom hostname. Default to null."
  default     = null
}
...
variable "create_tfe_custom_hostname_on_cis" {
  ...
  default     = false
  validation {
    condition     = (var.create_tfe_custom_hostname_on_cis == true && var.tfe_custom_hostname != null) || var.create_tfe_custom_hostname_on_cis == false
    error_message = "If var.create_tfe_custom_hostname_on_cis is true the value of var.tfe_custom_hostname cannot be null."
  }
}
